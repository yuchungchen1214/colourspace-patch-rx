...
void msgMetric(char* buf, int rVal, int gVal, int bVal, double x, double y, double Y)
{
    ostringstream sout;
...
void msgMeasure(char* buf, int rVal, int gVal, int bVal, int driftIdx = -1)
{
    ostringstream sout;
    sout << "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"       << endl; 
    sout << "<CS_RMC version=1>"                                << endl;
    ...
    for(int rc(0); rc < sz; ++rc )
    {
        for(int gc(0); gc < sz; ++gc )
        {
            for(int bc(0); bc < sz; ++bc )
            {
                ...
                cout << "Sending measure patch " << ct << " (" << ri << "," << gi << "," << bi << ")" << endl;
                memset(rspBuf, 2048, 0x00);
                msgMeasure(reqBuf, ri, gi, bi);
                cout << reqBuf << endl;
                ...
                memset(rspBuf, 2048, 0x00);
                msgMetric(reqBuf, ri, gi, bi, rx, ry, rY);
                cout << reqBuf << endl;
                sendCmd(sd, reqBuf, rspBuf);
